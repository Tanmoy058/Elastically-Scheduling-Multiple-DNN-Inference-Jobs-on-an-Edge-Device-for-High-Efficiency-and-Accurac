...
void take_input_ml_graph(){
    ifstream fin("ml_graph_inception_another.txt");
    ifstream fmemory("new_memory_calc.txt");
    //format:
    ...
    for(int i = 0; i<cur_stage_layers.size(); ++i){
        ...
        if(gg[my_layer].alpha_memory>my_copy[my_device].memory){
            ...
            for(auto eg: rev_adj[my_layer]){
                ...
                if(prev_device!=my_device)comm_time = comm_time+(tx*8.0)/(BW_matrix_master[prev_device]*1024.0*1024.0)+(tx*8.0)/(BW_matrix_master[my_device]*1024.0*1024.0);
            }
...
int main(){
    ...
    find_first_placement(tp);
    /*run_python();
    ifstream fin("from_c_output.txt");
    string s;
    while(fin>>s){
        cout<<s<<"\n";
    }*/
    ...
    for(it = P1.begin();it!=P1.end(); ++it){
        fout<<rev_vertex_map[it->first]<<" "<<it->second<<"\n";
        cout<<rev_vertex_map[it->first]<<" "<<it->second<<"\n";
    }
    fout.close();
    return 0;
}
